...
void setup() {
  ...
  //Java Mode
  cs = new Csoundo(this, "partikkelMorphOSC.csd");
  cs.run();
  ...
  for (int i=0;i<controllerList.size();i+=1) {
    println(controllerList.get(i));
  }
  mOSC = new MorphOSC(this);
  mOSC.addAllControllers(cp5);
}

void draw() {
  background(0);
  if (millis()-interpStartTime<xFadeTime*1000) {
    interpSliders(); //TODO: fix this hack, do not call continuously in draw.
  }
}
...
void initControllers() {
  ...
  int hPad = 5, vPad = 80, borderPad = hPad*2;
  int sliderW = width/(2*16)-hPad, sliderH= height/5;
  int sliderX = width/(2*16)+hPad;
  // Top buttons...
  int buttonW = 40, buttonH = 20;
  int yPos = borderPad;
  cp5.addToggle("On", buttonW*2, yPos, buttonW, buttonH);
  ...
  samp1.setPosition(borderPad, yPos)
  ...
  samp3.setPosition(borderPad, yPos+sliderH-buttonH)
    .setSize(sliderW, buttonH)
      .setRange(1, 7)
        .setMultiplier(1) // set the sensitifity of the numberbox
          .setDirection(Controller.VERTICAL) // change the control direction to left/right
  ...
  samp2.setPosition(sliderX*3, yPos)
    .setSize(sliderW, buttonH)
      .setRange(1, 7)
        .setMultiplier(1) // set the sensitifity of the numberbox
          .setDirection(Controller.VERTICAL) // change the control direction to left/right
  ...
  samp4.setPosition(sliderX*3, yPos+sliderH- buttonH)
    .setSize(sliderW, buttonH)
      .setRange(1, 7)
        .setMultiplier(1) // set the sensitifity of the numberbox
          .setDirection(Controller.VERTICAL) // change the control direction to left/right
